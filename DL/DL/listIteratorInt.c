...
int *findNext(IteratorInt it, int v){
	...
	while (it->currentNode->nextElement != v&&it->currentNode->next != NULL)            //查找
		it->currentNode = it->currentNode->next;
	if (it->currentNode->nextElement == v&&it->currentNode->next != NULL)
	{
		it->flag = 1;
		int* returnaddr = &(it->currentNode->nextElement);
		it->currentNode = it->currentNode->next;
		return returnaddr;
	}
	else
...
int *findPrevious(IteratorInt it, int v){
	...
	while (it->currentNode->prevElement != v&&it->currentNode->prev != NULL)
		it->currentNode = it->currentNode->prev;
	if (it->currentNode->prevElement == v&&it->currentNode->prev != NULL)
	{
		it->flag = -1;
		int* returnaddr = &(it->currentNode->prevElement);
		it->currentNode = it->currentNode->prev;
		return returnaddr;
	}
	else
