...
std::string AssetTesting::testDate() {
  bool failure{false};
  Date vDate{1, Date::January, 1999};
  if (!vDate.valid()) {
    _failMessages << "Date valid() valid test failed." << std::endl;
    failure = true;
  }
  Date ivDate{31, Date::February, 2016};
  if (ivDate.valid()) {
    _failMessages << "Date valid() invalid test failed." << std::endl;
    failure = true;
  }
  Date compare{1, Date::January, 1999};
  if (!(compare == vDate) || (compare != vDate)) {
    _failMessages << "Date equality and inequality test failed." << std::endl;
  }
...
Date AssetTesting::generateRandomDate() {
  time_t date{(*_dateDistribution)(_randomEngine)};
