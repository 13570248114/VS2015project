// new_compute5.cpp : 定义控制台应用程序的入口点。
//
...
bool isInTheSpecialVertexList(int x, int* specialVertex, int numOfSpecialVer)
{
	...
	for (int m = 1; m != Limit_num_nodes - 1; ++m)   //Limit_num_nodes为限制的节点数
	{
		for (int i = 0; i<List->num; ++i) //第i块
		{
			for (int j = 0;j < List->PointU->graph->data;++j)  //第j个节点
			{
				...
				if (dist[i][j] == pdist[i][j]&& !isInTheSpecialVertexList(j,specialVertex,numOfSpecialVer))
				{
